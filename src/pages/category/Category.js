...
const Category = ({ setPageTitle }) => {
  const categoryModalRef = useRef(null);
  const categoryModalEditRef = useRef(null);
  ...
  return loading ? (
    ...
    <PageComponent>
      <PageTitle
        title={<Title style={{ margin: 0 }}>Category</Title>}
        rightPanel={
          <MyButton
        ...
        columns={[
          {
            ...
            render: (_, item) => {
              ...
              return images.length ? (
                <Image width={100} height={100} src={images[0]} />
              ) : (
                <Image
                  width={100}
                  height={100}
            ...
            render: (_, item) => {
              return (
                <ConfirmActions
                  ...
                  onConfirm={async () => {
                    setLoading(true);
                    const result = await categoryActions.deleteCategoryItem(
                      item.idCategory
                    );
                    if (result.success) {
                      const reminingItems = tableData.list.filter(
                        (filItems) => filItems.idCategory !== item.idCategory
                      );
                      setTableData({
                        list: reminingItems,
                      ...
                      notifySuccess("You deleted your category successfully");
                    } else {
                      notifyError(result.errMsg);
                    }
                    setLoading(false);
            ...
            render: (_, item) => {
              return (
                <>
                  <EditOutlined
                    onClick={() => {
                      categoryModalEditRef?.current?.openModal();
                      setCategoryItem(item);
                    }}
                  />
                  <CategoryEditModal
                    ref={categoryModalEditRef}
                    item={categoryItem}
                    onSave={() => {
                      fill();
                    }}
